<?php
    ...
    if(count($session_info)>0){
        if(time()>session_info["expiry_time"]){
            ...
            $user_check_query = "SELECT balance FROM bank WHERE user_id='$username' LIMIT 1";
            $result = mysqli_query($db, $user_check_query);
            $balance = mysqli_fetch_assoc($result);
            
            if ($action === "deposit") {
                $balance = $balance + $amount;
                $user_check_query = "UPDATE bank set balance='$balance' WHERE user_id='$username' LIMIT 1";
                $result = mysqli_query($db, $user_check_query);
                print($balance);
            }
            ...
            elseif($action === "close"){
                $user_check_query = "DELETE FROM bank WHERE user_id='$username' LIMIT 1";
                $result = mysqli_query($db, $user_check_query);
            }
